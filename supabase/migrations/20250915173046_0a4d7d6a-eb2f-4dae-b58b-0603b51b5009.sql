...
-- ========================================
-- CONFIGURAÇÃO DE TRIGGERS LIMPOS
-- ========================================

-- DROP triggers existentes se houver
DROP TRIGGER IF EXISTS trigger_recalc_orcamento_insert ON public.itens_orcamento;
DROP TRIGGER IF EXISTS trigger_recalc_orcamento_update ON public.itens_orcamento;
DROP TRIGGER IF EXISTS trigger_recalc_orcamento_delete ON public.itens_orcamento;
DROP TRIGGER IF EXISTS trigger_aprovar_orcamento_update ON public.orcamentos;
DROP TRIGGER IF EXISTS trigger_rejeitar_orcamento_update ON public.orcamentos;
DROP TRIGGER IF EXISTS trigger_confirmar_venda_update ON public.vendas;
DROP TRIGGER IF EXISTS trigger_adicionar_saida_insert ON public.lancamentos_financeiros;
DROP TRIGGER IF EXISTS trigger_inativar_usuario_update ON public.usuarios;
DROP TRIGGER IF EXISTS audit_clientes_trigger ON public.clientes;
DROP TRIGGER IF EXISTS update_updated_at_clientes ON public.clientes;
DROP TRIGGER IF EXISTS update_updated_at_orcamentos ON public.orcamentos;
DROP TRIGGER IF EXISTS update_updated_at_pagamentos ON public.pagamentos;

-- Recriar triggers organizados
CREATE TRIGGER trigger_recalc_orcamento_insert
  AFTER INSERT ON public.itens_orcamento
  FOR EACH ROW EXECUTE FUNCTION public.recalc_valor_total_orcamento();

CREATE TRIGGER trigger_recalc_orcamento_update
  AFTER UPDATE ON public.itens_orcamento
  FOR EACH ROW EXECUTE FUNCTION public.recalc_valor_total_orcamento();

CREATE TRIGGER trigger_recalc_orcamento_delete
  AFTER DELETE ON public.itens_orcamento
  FOR EACH ROW EXECUTE FUNCTION public.recalc_valor_total_orcamento();

CREATE TRIGGER trigger_aprovar_orcamento_update
  AFTER UPDATE ON public.orcamentos
  FOR EACH ROW EXECUTE FUNCTION public.trigger_aprovar_orcamento();

CREATE TRIGGER trigger_rejeitar_orcamento_update
  AFTER UPDATE ON public.orcamentos
  FOR EACH ROW EXECUTE FUNCTION public.trigger_rejeitar_orcamento();

CREATE TRIGGER trigger_confirmar_venda_update
  AFTER UPDATE ON public.vendas
  FOR EACH ROW EXECUTE FUNCTION public.trigger_confirmar_venda();

CREATE TRIGGER trigger_adicionar_saida_insert
  AFTER INSERT ON public.lancamentos_financeiros
  FOR EACH ROW EXECUTE FUNCTION public.trigger_adicionar_saida();

CREATE TRIGGER trigger_inativar_usuario_update
...
CREATE TRIGGER update_updated_at_clientes
  BEFORE UPDATE ON public.clientes
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

CREATE TRIGGER update_updated_at_orcamentos
  BEFORE UPDATE ON public.orcamentos
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at();

CREATE TRIGGER update_updated_at_pagamentos
...
-- Adicionar todas as tabelas ao publication para realtime
